...
*/
/* --------------------------------------------------------------------------*/
static int vcodec_h_ioctl(int h, unsigned int cmd, unsigned int subcmd, unsigned long parameter)
{
    int r;
...
static inline int vcodec_video_es_init(vcodec_para_t *pcodec, int sched)
{
    ...
    if (pcodec->video_type == VFORMAT_AV1) {
        ...
        if ((pcodec->amld_enable) && (pcodec->video_type == VFORMAT_HEVC))
        ...
        if (pcodec->video_type == VFORMAT_H264 && pcodec->amld_enable)
